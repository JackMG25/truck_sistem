...
<div class="space-y-3">
    ...
    <div class="grid grid-cols-2 gap-3">
        ...
        <div>
            ...
            <input
                ...
                name="direccion"
                type="text"
                value="{{ old('direccion', $agencia->direccion) }}"
                class="w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm text-slate-800 placeholder:text-slate-400 focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-200"
    ...
    <div>
        ...
        <textarea
            ...
            rows="3"
            class="w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm text-slate-800 placeholder:text-slate-400 focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-200"
            placeholder="Notas importantes de la agencia"
        >{{ old('observaciones', $agencia->observaciones) }}</textarea>
        @error('observaciones')
            <p class="mt-1 text-xs text-rose-500">{{ $message }}</p>
        @enderror
    </div>
    ...
    <div class="flex items-center gap-2 pt-2">
        ...
        <a href="{{ route('agencias.index') }}" class="inline-flex items-center justify-center rounded-md border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-600 shadow-sm">
            Cancelar
        </a>
    </div>
</div>