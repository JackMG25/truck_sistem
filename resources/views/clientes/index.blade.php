<x-app-layout title="Clientes">
    ...
    <section class="space-y-4">
        <div class="rounded-[1.75rem] border border-slate-200 bg-white p-4 shadow-lg shadow-sky-100/70 sm:p-5">
            <div class="flex flex-row flex-wrap items-center justify-between gap-3">
                <div class="flex items-center gap-2">
                    <a href="{{ route('clientes.create') }}" class="inline-flex items-center justify-center gap-2 rounded-2xl bg-sky-600 px-4 py-3 text-sm font-semibold text-white shadow-lg shadow-sky-200">
                ...
                <form id="clientes-search-form" method="GET" action="{{ route('clientes.index') }}" class="flex w-full items-center gap-2 lg:max-w-md">
        ...
        @if ($clientes->isEmpty())
            <div class="rounded-[1.75rem] border border-dashed border-slate-300 bg-white p-8 text-center shadow-lg shadow-sky-100/70">
                <p class="text-lg font-semibold text-slate-800">No hay clientes para mostrar</p>
                <p class="mt-2 text-sm text-slate-500">Prueba con otra búsqueda o registra un nuevo cliente.</p>
            </div>
        @else
                ...
                @foreach ($clientes as $cliente)
                    <article class="rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-lg shadow-sky-100/70">
                        <div class="flex items-start justify-between gap-3">
                            <div>
                                <h3 class="text-sm font-semibold text-slate-800">{{ $cliente->nombre }}</h3>
                                <p class="mt-0.5 text-xs text-slate-500">{{ $cliente->telefono ?: 'Sin teléfono' }}</p>
                            </div>
                            <div class="flex items-center gap-2">
                                <a href="{{ route('clientes.edit', $cliente) }}" class="inline-flex h-8 w-8 items-center justify-center rounded-md border border-amber-200 bg-amber-50 text-amber-700 hover:bg-amber-100 sm:h-9 sm:w-9" aria-label="Editar cliente {{ $cliente->nombre }}">
                                ...
                                <form action="{{ route('clientes.destroy', $cliente) }}" method="POST" class="delete-form" data-nombre="{{ $cliente->nombre }}">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="inline-flex h-8 w-8 items-center justify-center rounded-md border border-rose-200 bg-rose-50 text-rose-700 hover:bg-rose-100" aria-label="Eliminar cliente {{ $cliente->nombre }}">
                                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" class="h-4 w-4">
                        ...
                        <div class="mt-3 grid gap-2 text-sm text-slate-600">
                            <div>
                                ...
                                <p class="mt-1 text-xs">{{ $cliente->direccion ?: 'Sin dirección registrada' }}</p>
                            </div>
                            <div>
                                <span class="block text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-400">Observaciones</span>
                                <p class="mt-1 text-xs whitespace-pre-line">{{ $cliente->observaciones ?: 'Sin observaciones' }}</p>
                            </div>
                        </div>
                    </article>
                @endforeach
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200 text-sm text-slate-700">
                        ...
                        <tbody class="divide-y divide-slate-200">
                            @foreach ($clientes as $cliente)
                                <tr>
                                    <td class="px-3 py-3 font-medium text-slate-800">{{ $cliente->nombre }}</td>
                                    <td class="px-3 py-3 text-sm">{{ $cliente->telefono ?: 'Sin teléfono' }}</td>
                                    <td class="px-3 py-3 text-sm">{{ $cliente->direccion ?: 'Sin dirección' }}</td>
                                    <td class="px-3 py-3 max-w-sm whitespace-pre-line text-sm text-slate-600">{{ $cliente->observaciones ?: 'Sin observaciones' }}</td>
                                    <td class="px-3 py-3">
                                        <div class="flex justify-end gap-2">
                                            <a href="{{ route('clientes.edit', $cliente) }}" class="inline-flex h-8 w-8 items-center justify-center rounded-md border border-amber-200 bg-amber-50 text-amber-700" aria-label="Editar cliente {{ $cliente->nombre }}">
                                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" class="h-4 w-4">
                                                    <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931ZM18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                                                </svg>
                                            </a>
                                            <form action="{{ route('clientes.destroy', $cliente) }}" method="POST" onsubmit="return confirm('¿Eliminar este cliente?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="inline-flex h-8 w-8 items-center justify-center rounded-md border border-rose-200 bg-rose-50 text-rose-700" aria-label="Eliminar cliente {{ $cliente->nombre }}">
                                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" class="h-4 w-4">
                                                        <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.021.166m-1.021-.165L18.16 19.673A2.25 2.25 0 0 1 15.916 21H8.084a2.25 2.25 0 0 1-2.245-1.327L4.772 5.79m14.456 0A48.108 48.108 0 0 0 3.75 5.79m5.25 0V4.875C9 3.839 9.84 3 10.875 3h2.25C14.16 3 15 3.84 15 4.875V5.79" />
                                                    </svg>
                                                </button>
    ...
    <script>
        (function(){
            const form = document.getElementById('clientes-search-form');
            if (!form) return;
        ...
        (function(){
            document.querySelectorAll('.delete-form').forEach(function(form){
                form.addEventListener('submit', function(e){
                    e.preventDefault();
                    const nombre = form.getAttribute('data-nombre') || 'este cliente';
                    Swal.fire({
                        title: '¿Eliminar?',
