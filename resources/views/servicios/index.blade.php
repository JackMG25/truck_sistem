                ...
                @foreach ($servicios as $servicio)
                    <article class="rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-lg shadow-sky-100/70">
                        ...
                        <div class="mt-3 flex flex-wrap gap-2 text-[11px] font-semibold">
                            ...
                            @if ($servicio->estaCompletamentePagado())
                                <span class="inline-flex rounded-full border px-2 py-1 {{ $estadoPagoClasses['PAGADO'] }}">PAGADO</span>
                            @else
                                <span class="inline-flex rounded-full border px-2 py-1 {{ (float) ($servicio->total_pagado ?? 0) > 0 ? $estadoPagoClasses['PARCIAL'] : $estadoPagoClasses['PENDIENTE'] }}">
                                    Debe S/ {{ number_format($servicio->saldo_pendiente, 2, '.', ',') }}
                                </span>
                            @endif
                        </div>
                        ...
                        <div class="mt-3 grid gap-2 text-sm text-slate-600">
                            ...
                            <a
                                href="{{ route('servicios.pagos', $servicio) }}"
                            ...
                            >
                                ...
                                <span class="rounded-full bg-white px-2 py-0.5 text-[11px] text-sky-700">{{ $servicio->pagos_count }}</span>
                            </a>
                        </div>
                    </article>
                @endforeach
                                ...
                                <tr>
                                    ...
                                    <td class="px-3 py-3 align-top">
                                        <div class="flex flex-col gap-2 text-[11px] font-semibold">
                                            ...
                                            @if ($servicio->estaCompletamentePagado())
                                                <span class="inline-flex w-fit rounded-full border px-2 py-1 {{ $estadoPagoClasses['PAGADO'] }}">PAGADO</span>
                                            @else
                                                <span class="inline-flex w-fit rounded-full border px-2 py-1 {{ (float) ($servicio->total_pagado ?? 0) > 0 ? $estadoPagoClasses['PARCIAL'] : $estadoPagoClasses['PENDIENTE'] }}">
                                                    Debe S/ {{ number_format($servicio->saldo_pendiente, 2, '.', ',') }}
                                                </span>
                                            @endif
                                        </div>
